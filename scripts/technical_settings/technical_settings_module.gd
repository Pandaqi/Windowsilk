...
func update_to_config():
	var val = GlobalConfig.get_config_val(sec, nm)
...
func _on_HSlider_value_changed(value):
	...
	GlobalConfig.update_config_val({ "sec": sec, "name": nm}, value)

func _on_Checkbox_toggled(button_pressed):
	GlobalAudio.play_static_sound("button")
	GlobalConfig.update_config_val({ "sec": sec, "name": nm}, button_pressed)
